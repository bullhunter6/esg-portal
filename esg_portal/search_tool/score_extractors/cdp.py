    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if response.status_code >= 500:
                return [{"error": f"HTTP error: {response.status_code}", "source": "CDP"}]
            else:
                return [{"Response Score Band": "-", "source": "CDP"}]
    except Exception as e:
        print(f"Error fetching CDP data: {e}")
        return [{"error": f"Exception: {str(e)}", "source": "CDP"}]
...
def cdp_score(company_name, year):
    ...
    if data:
        ...
        if len(data) == 1 and isinstance(data[0], dict) and "error" in data[0]:
            ...
            # Only return serious system errors, not search-related errors
            if "HTTP error: 5" in error_msg or "network" in error_msg.lower() or "timeout" in error_msg.lower():
                ...
                return [{"Response Score Band": "-", "source": "CDP"}]  # All other cases = "-"
        # Check if we have actual data
        if any(isinstance(item, dict) and "Source" in item for item in data):
            return data
        else:
            return [{"Response Score Band": "-", "source": "CDP"}]
    else:
